...
2.  **在 TabBarView 等场景中**：当使用 `TabBarView` 或 `PageView` 时，只有当前页面是可见的。这是一个需要开发者手动管理，以暂停后台页面活动的典型场景。
...
-   **`didPushNext()`**：当一个新路由被推到当前路由之上时触发。这意味着当前页面被覆盖了，应该**暂停**。
-   **`didPopNext()`**：当顶层路由被弹出，使当前路由重新可见时触发。这意味着当前页面恢复可见，应该**恢复**。
...
-   **`AppLifecycleState.hidden`** (或 `paused`, `inactive`)：当应用切换到后台或被电话等系统事件中断时触发。此时整个应用都不可见，应该**暂停**。
-   **`AppLifecycleState.resumed`**：当应用重新回到前台时触发。此时应用恢复可见，应该**恢复**。
...
`TickerMode` 是一个 `InheritedWidget`，它允许你显式地**启用或禁用**其子树中所有 `Ticker` 的活动。
...
-   当 `enabled` 为 `true` 时，子树中的 `Ticker` 可以正常“心跳”，动画会播放。
-   当 `enabled` 为 `false` 时，子树中的 `Ticker` 会被“静音”（muted），心跳停止，所有动画都会暂停。
...
-   对于**当前可见的 Tab**，它会用 `TickerMode(enabled: true, ...)` 包裹。
-   对于**其他不可见的 Tab**，它会用 `TickerMode(enabled: false, ...)` 包裹。
...
`PauseAwareController` 的设计是完全开放的。你可以实现自己的 `ReferPauseProvider` 来应对任何特殊的暂停逻辑。
...
1.  **创建自定义 Provider**：
    ```dart
    class ConnectionPauseProvider extends ReferPauseProvider {
...
2.  **添加到 ViewModel**：
    你可以通过 `ViewModelStateMixin` 的 `addPauseProvider` 方法，在 `initState` 中动态添加你的自定义 Provider。
...
`view_model` 的 `Pause` 机制提供了一套强大而灵活的解决方案
